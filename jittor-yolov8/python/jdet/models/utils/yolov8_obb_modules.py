...
class Add(nn.Module):
    """M2D-LIF naive half-add fusion: 0.5*x0 + 0.5*x1."""
    def execute(self, x):
        return 0.5 * x[0] + 0.5 * x[1]


class IN(nn.Module):
    """Identity entry node used by M2D-LIF YAML."""
    def execute(self, x):
        return x


class Multiin(nn.Module):
    """Split a 6-channel RGB+IR tensor into RGB or IR 3-channel branch."""
    def __init__(self, out=1):
        super().__init__()
        self.out = int(out)

    def execute(self, x):
        return x[:, :3, :, :] if self.out == 1 else x[:, 3:6, :, :]


class LIF(nn.Module):
    """
    Local Illumination-aware module from M2D-LIF.
    It predicts a 1-channel illumination/quality map at stride 8.
    For 640x640 input the output is 80x80.
    """
    def __init__(self):
        super().__init__()
        ...
        self.relu = nn.ReLU()
        self.pool = nn.Pool(kernel_size=2, stride=2, op='mean')
...
class LIFAdd(nn.Module):
    """
    M2D-LIF weighted feature fusion.
    Input order follows the original YAML: [IR feature, RGB feature, LIF weight].
    W_rgb = beta * clamp((B - 0.31) / 0.63, max=0.5) + 0.5, beta=0.4.
    """
    def __init__(self, layer):
        super().__init__()
        self.layer = int(layer)
        self.pool_layer4 = nn.Pool(kernel_size=2, stride=2, op='mean')
        self.pool_layer5 = nn.Pool(kernel_size=4, stride=4, op='mean')
        self.beta = 0.4

    def execute(self, x):
        x_ir, x_rgb, weight = x[0], x[1], x[2]
        weight = (weight - 0.31) / 0.63
        weight = jt.minimum(weight, jt.ones_like(weight) * 0.5)
        weight = self.beta * weight + 0.5
        if self.layer == 4:
            weight = self.pool_layer4(weight)
        elif self.layer == 5:
            weight = self.pool_layer5(weight)
        return weight * x_rgb + (1.0 - weight) * x_ir
