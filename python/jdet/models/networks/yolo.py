...
class ModelEMAWraper(nn.Module):
    ...
    def state_dict(self, recurse=True):
        if self.ema_hooked:
            return self.ema.ema.state_dict(recurse=recurse)
        else:
            return self.model.state_dict(recurse=recurse)
...
class YOLO(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(m, Detect):
            ...
            for i, x in enumerate(outs):
                h = x.shape[-2]
                stride_i = float(s) / float(h)
                raw_strides.append(stride_i)

            # 关键：不要转 jt.array / jt.Var，直接保留 Python float list
            m.stride = raw_strides

            # 原来是：m.anchors /= m.stride.view(-1, 1, 1)
            # 现在改成逐层除以 Python float，避免构造 stride tensor
            for i, stride in enumerate(m.stride):
                m.anchors[i] = m.anchors[i] / float(stride)
        ...
        if isinstance(self.stride, (list, tuple)):
            gs = int(max(self.stride))
        else:
            stride_max = self.stride.max()
            if hasattr(stride_max, "item"):
                stride_max = stride_max.item()
            gs = int(stride_max)
        nl = self.model[-1].nl
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):
            # s 可能是 Python float，也可能是 Jittor Var，统一兼容
            if hasattr(s, "item"):
                s = float(s.item())
            else:
                s = float(s)
            ...
            obj_prior = 8.0 / (640.0 / s) ** 2
            obj_prior = max(obj_prior, 1e-9)
            b[:, 4] += math.log(obj_prior)

            if cf is None:
                cls_prior = 0.6 / (m.nc - 0.99)
                cls_prior = max(cls_prior, 1e-9)
                b[:, 5:] += math.log(cls_prior)
            else:
                b[:, 5:] += jt.log(cf / cf.sum())
